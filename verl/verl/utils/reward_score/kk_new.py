...
def validate_response_structure(processed_str: str) -> bool:
    ...
    if (positions['think_start'] > positions['think_end'] or
        positions['answer_start'] - (positions['think_end'] + len('</think>')) > 1 or
...
def get_thinking_reward(processed_str: str, expected_names: List) -> float:
    ...
    words = re.findall(r'\b\w+\b', think_content)
    word_count = len(words)
    
    thinking_factor += int(word_count / 100.0) * 1e-5
    ...
    if thinking_factor >= 1.0:
        reflection_tokens = [
            "wait", "verify", "yet", "re-evaluate", "review",
            "∨", "\\u2228",  # OR
            "∧", "\\u2227",  # AND
            "¬", "\\u00AC",  # NOT
        ]
        ...
        for token in reflection_tokens:
            if token in text_lower:
                print(f"  Reflection token detected: {token}")
...
def compute_score(
    ...
    answer_score = 0
    thinking_factor = 1.0
    if answer_text:  # and format_correct and valid_response_length > 500:
    ...
    length_score = 0
    if valid_response_length <= 100 or valid_response_length >= config.data.max_response_length:
        length_score = -1.0
    ...
    total_score =  answer_score + length_score  # + format_score + language_mix_score
